...
- Fetches the audit script from a pinned tag (`v2.12.3` by default)
...
The template uses `AUDIT_REF: 'v2.12.3'` to pin to a released version. Bump this when you upgrade. Alternatives:
...
steps:
  ...
  - bash: |
      curl -fsSL https://raw.githubusercontent.com/Nerdy-Q/claude-power-pages-plugins/v2.12.3/plugins/pp-permissions-audit/skills/pp-permissions-audit/scripts/audit.py \
           -o /tmp/audit.py
